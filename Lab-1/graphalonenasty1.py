...
plt.plot([a_t[3], 13.5], [a[3], a[3]], linestyle='--', linewidth=0.5, color='black')
plt.plot([b_t[3], 13.5], [b[3], b[3]], linestyle='--', linewidth=0.5, color='black')
plt.plot([13.47, 13.47], [b[3], a[3]], linewidth=1, color='black')
plt.plot([13.47, 13.476], [a[3], a[3] - 0.18], linewidth=1, color='black')
plt.plot([13.47, 13.464], [a[3], a[3] - 0.18], linewidth=1, color='black')
plt.plot([13.47, 13.476], [b[3], b[3] + 0.18], linewidth=1, color='black')
plt.plot([13.47, 13.464], [b[3], b[3] + 0.18], linewidth=1, color='black')
for i in range(len(b)):
    plt.plot([a_t[i] + 0.01, b_t[i] - 0.01], [low + 0.09, low + 0.09], linewidth=1, color='black')
    plt.plot([a_t[i] + 0.01, a_t[i] + 0.03], [low + 0.09, low + 0.12], linewidth=1, color='black')
    plt.plot([a_t[i] + 0.01, a_t[i] + 0.03], [low + 0.09, low + 0.06], linewidth=1, color='black')
    plt.plot([b_t[i] - 0.01, b_t[i] - 0.03], [low + 0.09, low + 0.12], linewidth=1, color='black')
    plt.plot([b_t[i] - 0.01, b_t[i] - 0.03], [low + 0.09, low + 0.06], linewidth=1, color='black')
for i in range(len(b)):
    ax.text((a_t[i] + b_t[i])/2, low + 0.12, f"T", horizontalalignment='center', fontsize=13)
ax.text(13.5, (a[3] + b[3]) / 2, f"a-b", verticalalignment='center', fontsize=13)
tick_spacing_x = 0.1
tick_spacing_y = 0.1
